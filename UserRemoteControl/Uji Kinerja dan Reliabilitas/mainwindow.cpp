    ...
    , previousSliderState(true)
{
    ...
    performanceUpdateTimer = new QTimer(this);
    connect(performanceUpdateTimer, &QTimer::timeout, this, &MainWindow::updatePerformanceMetrics);
    performanceUpdateTimer->start(1000); // Update every second

    // Add a button to start the performance test
    QPushButton *testButton = new QPushButton("Start Performance Test", this);
    ui->statusBar->addPermanentWidget(testButton);
    connect(testButton, &QPushButton::clicked, this, &MainWindow::startPerformanceTest);
...
void MainWindow::readSerial() {
    ...
    while (serialBuffer.contains('\n')) {
        ...
        ++totalPackets;
        totalTransmissionTime += packetTimer.elapsed();
        totalLatency += packetTimer.elapsed();
    }
}
...
void MainWindow::initializePerformanceMetrics() {
    totalPackets = 0;
    errorPackets = 0;
...
void MainWindow::updatePerformanceMetrics() {
    // qint64 uptime = uptimeTimer.elapsed();

    // Only calculate values that are actually used
    // Remove or comment out unused calculations

    // double avgTransmissionTime = totalPackets > 0 ? static_cast<double>(totalTransmissionTime) / totalPackets : 0;
    // double errorRate = totalPackets > 0 ? static_cast<double>(errorPackets) / totalPackets * 100 : 0;
    // double avgLatency = totalPackets > 0 ? static_cast<double>(totalLatency) / totalPackets : 0;
    // double throughput = uptime > 0 ? (totalPackets * 8) / (uptime / 1000.0) : 0; // in bps
    // double uptimePercentage = (uptime / 1000.0) / (24 * 60 * 60) * 100; // Assuming 24 hours of operation
...
void MainWindow::updatePerformanceDisplay() {
    ...
    double errorRate = totalPackets > 0 ? static_cast<double>(errorPackets) / totalPackets * 100 : 0;
    double avgLatency = totalPackets > 0 ? static_cast<double>(totalLatency) / totalPackets : 0;
    double throughput = uptimeTimer.elapsed() > 0 ? (totalPackets * 8) / (uptimeTimer.elapsed() / 1000.0) : 0;
    double uptimePercentage = (uptimeTimer.elapsed() / 1000.0) / (24 * 60 * 60) * 100;

    ui->avgTransmissionTimeLabel->setText(QString::number(avgTransmissionTime, 'f', 2) + " ms");
    ui->errorRateLabel->setText(QString::number(errorRate, 'f', 2) + "%");
    ui->avgLatencyLabel->setText(QString::number(avgLatency, 'f', 2) + " ms");
    ui->throughputLabel->setText(QString::number(throughput, 'f', 1) + " Kbps");
    ui->uptimeLabel->setText(QString::number(uptimePercentage, 'f', 2) + "%");
}

void MainWindow::startPerformanceTest() {
    initializePerformanceMetrics();
